...
file_broker = st.sidebar.file_uploader(
    "2. Broker Statement", type=["csv", "xlsx", "xls"]
)
file_pngs = st.sidebar.file_uploader(
    "3. Top 70 Chart Images (.jpg/.png)",
    type=["png", "jpg", "jpeg"],
...
top_70_png_universe = (
    []
)  # Store the exact symbols from the 70 PNGs
  ...
  try:
    ...
    for col in df_reg.columns:
      ...
      if "symbol" in c or "ticker" in c:
        reg_map[col] = "Symbol"
      elif "type" in c or "zone" in c:
        reg_map[col] = "Type"
      elif "sl" in c or "harmonic" in c or "stop" in c:
        reg_map[col] = "SL"
      elif "tp" in c or "kinetic" in c or "target" in c:
        reg_map[col] = "TP"
    ...
    if "Symbol" in df_reg.columns:
      df_reg["Symbol"] = (
          df_reg["Symbol"].astype(str).str.upper().str.strip()
      )
      df_reg = (
          df_reg.drop_duplicates(subset=["Symbol"], keep="first")
    ...
    # --- PARSE BROKER STATEMENT (CSV & EXCEL SAFE) ---
    file_name_lower = file_broker.name.lower()

    if file_name_lower.endswith(".csv"):
      df_broker_raw = pd.read_csv(file_broker, header=None).fillna("")
    else:
      try:
        df_broker_raw = pd.read_excel(file_broker, header=None).fillna("")
      except ImportError:
        st.sidebar.error(
            "⚠️ Excel support missing on cloud. Please upload your broker"
            " statement as a **.csv** file!"
        )
        st.stop()
    ...
    for i, row in df_broker_raw.head(30).iterrows():
      ...
      if (
          "security name" in row_text
          or "trade date" in row_text
    ...
    if "Symbol" not in df_broker.columns and "Name" in df_broker.columns:
      df_broker["Symbol"] = (
          df_broker["Name"].astype(str).str.split(" ").str[0].str.upper()
      )

      reg_symbols = (
          df_reg["Symbol"].unique() if "Symbol" in df_reg.columns else []
      )
      for idx, row in df_broker.iterrows():
        b_sym = str(row.get("Symbol", ""))
        if b_sym in reg_symbols:
          continue
        matches = [
            r
            for r in reg_symbols
            if str(r).startswith(b_sym) or b_sym.startswith(str(r))
        ]
        if matches:
    ...
    if "Qty" in df_broker.columns:
      df_broker["Qty"] = (
          pd.to_numeric(df_broker["Qty"], errors="coerce").fillna(0)
      )
    else:
      df_broker["Qty"] = 0
    ...
    if "Price" in df_broker.columns:
      df_broker["Price"] = (
          pd.to_numeric(df_broker["Price"], errors="coerce").fillna(0)
      )
    else:
      df_broker["Price"] = 0
    ...
    if "Value" in df_broker.columns:
      df_broker["Value"] = (
          pd.to_numeric(df_broker["Value"], errors="coerce").fillna(0)
      )
    else:
      df_broker["Value"] = df_broker["Qty"] * df_broker["Price"]
    ...
    st.sidebar.success(
        f"✅ Extracted {len(top_70_png_universe)} symbols from PNGs & Processed"
        f" {len(traded_images_b64)} charts!"
    )
